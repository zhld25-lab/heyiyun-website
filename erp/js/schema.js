...
function bespoke(){
    ...
    return {
        ...
        cost:[ fProject(), F("subject","成本科目",{type:"select",options:["人工","材料","机械","分包","间接费","其他"],col:true,filter:true}),
            ...
            fApproval(), fStatus(["待付款","部分付款","已付款"],"付款状态","待付款"), F("remark","备注",{type:"textarea",full:true}) ],
        fin_income:[ fProject(), F("contract","关联合同",{col:true,filter:true}), F("party","付款单位",{col:true}),
            fAmount("收款金额(万元)"), F("method","收款方式",{type:"select",options:["银行转账","承兑汇票","现金","其他"]}),
            F("date","收款日期",{type:"date",col:true,date:true}), fApproval(), fStatus(["待到账","已到账"],"到账状态","待到账"), F("remark","备注",{type:"textarea",full:true}) ],
        ...
        i_attach:[ F("title","文件名",{required:true,full:true,col:true}), F("ftype","类型",{col:true,filter:true}),
            F("size","大小",{col:true,align:"right"}), F("uploader","上传人",{col:true,filter:true}),
            F("date","上传时间",{col:true}), F("folder","归档月份",{col:true,filter:true}), F("url","原始路径",{full:true}) ],
